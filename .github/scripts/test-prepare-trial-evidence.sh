...
mkdir -p "${compressible_case}/scan-work/evidence"
truncate -s "$((payload_cap_bytes + 1048576))" \
  "${compressible_case}/scan-work/evidence/complete-evidence.bin"
compressible_digest="$(sha256sum "${compressible_case}/scan-work/evidence/complete-evidence.bin" | cut -d' ' -f1)"
(
  cd "${compressible_case}"
  GITHUB_OUTPUT=outputs.txt \
    GITHUB_STEP_SUMMARY=summary.md \
    bash "${helper}" evidence/archive.tar.gz 'Compressible evidence' scan-work/evidence
)
...
extracted_digest="$(sha256sum "${compressible_case}/extracted/scan-work/evidence/complete-evidence.bin" | cut -d' ' -f1)"
[[ "${extracted_digest}" == "${compressible_digest}" ]]

overflow_case="${test_root}/overflow"
mkdir -p "${overflow_case}/scan-work/evidence"
head -c "$((payload_cap_bytes + 1048576))" /dev/urandom \
  > "${overflow_case}/scan-work/evidence/complete-evidence.bin"
(
  cd "${overflow_case}"
  GITHUB_OUTPUT=outputs.txt \
    GITHUB_STEP_SUMMARY=summary.md \
    bash "${helper}" evidence/archive.tar.gz 'Overflow evidence' scan-work/evidence
)
...
if (
  cd "${missing_case}"
  bash "${helper}" evidence/archive.tar.gz 'Missing evidence' scan-work/evidence
); then
...
if (
  cd "${traversal_case}"
  bash "${helper}" evidence/archive.tar.gz 'Traversal evidence' ../scan-work/evidence
); then
