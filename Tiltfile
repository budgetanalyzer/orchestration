...
LOCAL_API_DOCS_RELEASE_METADATA = '.tilt/api-docs/release-metadata.json'

local("""
set -eu
mkdir -p .tilt/api-docs
python3 - <<'PY'
import json
import subprocess
...
root = Path.cwd()
parent = root.parent
source_path = root / "kubernetes/production/docs-aggregator/release-metadata.json"
output_path = root / ".tilt/api-docs/release-metadata.json"
...
def local_source_state(artifact):
    source_repo = artifact.get("sourceRepository", "")
    repo = parent / source_repo
    manifest_ref = artifact.get("sourceRef", "")
    manifest_commit = artifact.get("sourceCommit", "")
    manifest_tag = manifest_ref.removeprefix("refs/tags/")
    ...
    if not repo.is_dir():
        return {
            ...
            "matchesManifestTag": False,
        }

    head_commit = git(repo, "rev-parse", "HEAD")
    dirty = bool(git(repo, "status", "--porcelain", "--untracked-files=normal"))
    head_tags = git(repo, "tag", "--points-at", "HEAD").splitlines()
    head_tag = head_tags[0] if head_tags else ""
    manifest_tag_commit = ""

    if manifest_tag:
        manifest_tag_commit = git(repo, "rev-parse", "--verify", f"refs/tags/{manifest_tag}^{{}}")

    matches_manifest_tag = bool(
        head_commit
        and manifest_tag_commit
        and head_commit == manifest_tag_commit
        and (not manifest_commit or head_commit == manifest_commit)
    )

    if dirty and matches_manifest_tag:
        status = "modified"
        label = "modified from manifest tag"
    elif dirty:
        status = "modified"
        label = "modified local head"
    elif matches_manifest_tag:
        status = "matches-manifest-tag"
        label = "matches manifest tag"
    elif head_commit and manifest_commit and head_commit == manifest_commit:
        status = "matches-manifest-commit"
        label = "matches manifest commit"
    elif head_tag:
        status = "different-tag"
        label = f"different tag {head_tag}"
    elif head_commit:
        status = "untagged"
        label = "untagged local head"
    else:
        status = "unknown"
        label = "unknown"
    ...
    return {
        ...
        "headTag": head_tag,
        "manifestTag": manifest_tag,
        "matchesManifestTag": matches_manifest_tag,
    }
...
deployment = metadata.setdefault("deployment", {})
deployment["productionBaselineId"] = deployment.get("id", "")
deployment["id"] = "tilt-local-development"
deployment["environment"] = "local-development"
deployment["runtime"] = "tilt"
...
configmap_create(
    ...
    from_file=[
        ...
        'release-metadata.json=' + LOCAL_API_DOCS_RELEASE_METADATA,
