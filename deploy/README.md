...
| `deploy/scripts/25-deploy-oci-release.sh` | Lower-level OCI desired-state applier. It requires a schema v2 `--deployment-manifest`, validates it against the checked-in production baseline, runs the static gate, captures pre/post cluster snapshots, applies the managed production app set, waits for rollouts, and verifies live pods against the manifest. | Use directly only when replaying an explicit reviewed desired-state manifest. |
