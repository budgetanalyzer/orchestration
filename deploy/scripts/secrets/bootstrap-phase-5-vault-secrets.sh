...
usage() {
    cat <<'EOF'
Usage: ./deploy/scripts/secrets/bootstrap-phase-5-vault-secrets.sh [options]
...
Creates the OCI Vault secret inventory for all plain-text secrets:
  ...
  - stores the generated values in an operator-only file outside the repo so
    deploy/scripts/secrets/update-rabbitmq-definitions-secret.sh can render the RabbitMQ definitions
...
# Generated by deploy/scripts/secrets/bootstrap-phase-5-vault-secrets.sh
...
main() {
    ...
    phase4_warn "next step: run deploy/scripts/secrets/update-rabbitmq-definitions-secret.sh using the RabbitMQ passwords in ${GENERATED_ENV_FILE}"
}
