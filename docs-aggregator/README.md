...
- `release-metadata.json` is generated by
  `deploy/scripts/release/prepare-oci-manifest-from-current-stack.sh --resolve-images`
  from the reviewed schema v2 production deployment manifest for OCI; local
  Tilt generates its own ignored copy under `.tilt/api-docs/`.
