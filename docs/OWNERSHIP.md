...
| Dependency automation operating policy | [docs/dependency-automation.md](dependency-automation.md) | Keep activation, cost, bot ownership, and triage detail there; the rollout plan owns implementation phases and the preserved review remains the historical benchmark. |
...
| Host-published local ingress CA contract | [docs/development/local-environment.md](development/local-environment.md) | Setup and agent docs may name the published file and consumer command, but keep certificate publication, trust ownership, and troubleshooting detail here. |
| Autonomous agent execution and local credential trust boundary | [docs/architecture/autonomous-ai-execution.md](architecture/autonomous-ai-execution.md) | Plans and setup docs may summarize the boundary, but local-only agent authority, credential exposure assumptions, and production exclusions belong there. |
