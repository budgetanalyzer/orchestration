...
- Phase 6 Session 2 is implemented in-repo: `/api/docs` now serves pinned Swagger UI `5.11.0` assets from the same origin through a local static-asset image and init-container copy step, the page bootstrap moved to external JS/CSS files with no inline code, the shared docs UI explicitly disables submit methods so "Try it out" stays off independent of upstream defaults, and unlisted `/api/docs/*` requests now fail closed with docs-scoped strict-CSP `404`s instead of falling through to the frontend SPA.
- Phase 6 Session 3 is implemented in-repo as a stop-gate audit: `./scripts/dev/audit-phase-6-session-3-frontend-csp.sh` rebuilds the sibling production-smoke bundle and checks the repo-owned strict-CSP prerequisites. As of March 26, 2026, that static audit passes after the sibling frontend removed the inline-style and `sonner` blockers. Manual browser-console validation is still required. See [Phase 6 Session 3 audit](../plans/security-hardening-v2-phase-6-session-3-frontend-csp-audit.md).
- Phase 6 Session 4 is implemented in-repo: `nginx-gateway` now splits the relaxed Vite/HMR CSP from the strict production-oriented policy using checked-in include files, keeps the relaxed policy on the live dev routes, enforces a strict policy without `'unsafe-inline'` or `'unsafe-eval'` on `/_prod-smoke/` and `/api/docs`, and re-applies the full security header set inside the docs/download locations so route-specific `add_header` directives do not drop the non-CSP headers.
...
- Phase 6 Session 7 is implemented in-repo: before enabling `real_ip_*`, the live NGINX capture showed `remote_addr=127.0.0.6`, `xff="<caller>,<downstream-hop>"`, and `xrealip="-"`, which proved the trust anchor is the pod-local Envoy sidecar hop rather than a guessed cluster CIDR. `nginx-gateway` now trusts only `127.0.0.0/8` for `real_ip_header X-Forwarded-For`, keeps `real_ip_recursive on`, logs both the derived client identity and the trusted proxy hop, and therefore keys API `limit_req` buckets on the ingress-appended downstream client hop instead of the proxy sidecar address. `./scripts/dev/verify-phase-6-session-7-api-rate-limit-identity.sh` proves forged external `X-Forwarded-For` values cannot pick a new bucket and that different downstream clients do not share one bucket.
...
- Phase 6 Session 9 is implemented in-repo: `./scripts/dev/verify-phase-6-edge-browser-hardening.sh` now combines checked-in dev/strict CSP contract checks, live header verification for `/`, `/_prod-smoke/`, and `/api/docs`, negative probes that unknown `/api/docs/*` paths and unvendored sourcemap requests fail closed under the strict docs CSP, direct auth-edge throttling coverage for `/login`, `/auth/*`, `/logout`, and `/login/oauth2/*`, the Session 3 frontend CSP audit, the Session 7 API rate-limit identity verifier, and the full Phase 5 regression cascade. Manual browser-console validation on `/_prod-smoke/` and `/api/docs` remains required before Phase 6 can be declared complete.

Operationally, those ingress-facing policies depend on the rendered gateway label `gateway.networking.k8s.io/gateway-name=istio-ingress-gateway` and the rendered principal `cluster.local/ns/istio-ingress/sa/istio-ingress-gateway-istio`. The egress `DestinationRule` uses `tls.mode: DISABLE` on the workload-to-egress hop so the original external TLS/SNI reaches the egress gateway's `PASSTHROUGH` listener; external TLS remains end-to-end, but that intra-cluster hop is not additionally wrapped in mesh mTLS. `./scripts/dev/verify-security-prereqs.sh` proves the Phase 0 platform baseline. Treat Phase 3 as complete only after `./scripts/dev/verify-phase-3-istio-ingress.sh` and the live validation checklist pass. Treat Phase 6 as complete only after `./scripts/dev/verify-phase-6-edge-browser-hardening.sh` passes and the manual browser-console validation on `/_prod-smoke/` and `/api/docs` is done.
