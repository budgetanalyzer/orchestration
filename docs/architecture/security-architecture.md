...
- Local platform hardening Phase 0 is implemented: Kind uses Calico instead of `kindnet`, namespace PSA labels are staged in `warn`/`audit` for application and infrastructure namespaces, `istio-system` enforces `privileged` because the `istio-cni` DaemonSet runs there, and Kyverno plus a smoke policy are installed and verifiable.
