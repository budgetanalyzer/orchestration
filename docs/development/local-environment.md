...
- Virtual host: `/`
- The `currency-service` AMQP user is scoped to the `exchange-rate.import.requested` exchange, the `exchange-rate.import.requested.exchange-rate-import-service` queue and DLQ, server-named reply queues, `amq.default`, and `DLX`. The service queue and DLQ are included in `write` permission because RabbitMQ checks `write` during queue declaration. The former `currency.created` exchange and queue are intentionally outside the allow-list.
