...
- The RabbitMQ `currency-service` resource allow-list now follows the
  ...
  `budget-analyzer-rabbitmq-definitions` with
  `deploy/scripts/12-update-rabbitmq-definitions-secret.sh` before deploying
  the matching `currency-service` image.
...
2. Apply production secrets and IDP config if the upgrade changes non-secret
   ...
   Vault `budget-analyzer-rabbitmq-definitions` secret before this step, then
