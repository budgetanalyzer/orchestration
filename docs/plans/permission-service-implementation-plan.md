...
### Add `deletedBy` to SoftDeletableEntity

The current `SoftDeletableEntity` has `deleted` and `deletedAt` but lacks `deletedBy`. For permission management, we need to track WHO performed deletions.

**Changes to `/workspace/service-common/service-core/src/main/java/org/budgetanalyzer/core/domain/SoftDeletableEntity.java`:**
...
public abstract class SoftDeletableEntity extends AuditableEntity {
    ...
    @Column(length = 50)
    private String deletedBy;  // NEW FIELD

    public void markDeleted(String deletedBy) {  // UPDATED SIGNATURE
        this.deleted = true;
    ...
    public void restore() {
        this.deleted = false;
        this.deletedAt = null;
        this.deletedBy = null;  // CLEAR ON RESTORE
    }
...
- Existing soft-deleted records will have `deleted_by = NULL` (acceptable for historical data)
...
INSERT INTO roles (id, name, description) VALUES
    ('SYSTEM_ADMIN', 'System Administrator', 'Platform administration - manages roles, permissions, and can assign admin roles. Cannot be assigned via API.'),
    ('ORG_ADMIN', 'Organization Administrator', 'Organization administration - manages users and can assign basic roles within organization'),
    ('MANAGER', 'Manager', 'Team oversight - can view team data and approve operations'),
    ('ACCOUNTANT', 'Accountant', 'Professional access - can manage delegated user accounts'),
    ('AUDITOR', 'Auditor', 'Compliance access - read-only with full visibility'),
    ('USER', 'User', 'Self-service - access to own resources only');
...
INSERT INTO permissions (id, name, resource_type, action) VALUES
    ...
    ('transactions:read', 'Read Transactions', 'transaction', 'read'),
    ('transactions:write', 'Write Transactions', 'transaction', 'write'),
    ('transactions:delete', 'Delete Transactions', 'transaction', 'delete'),
    ('transactions:approve', 'Approve Transactions', 'transaction', 'approve'),
    ('transactions:bulk', 'Bulk Transaction Operations', 'transaction', 'bulk'),

    -- Account management
    ('accounts:read', 'Read Accounts', 'account', 'read'),
    ('accounts:write', 'Write Accounts', 'account', 'write'),
    ('accounts:delete', 'Delete Accounts', 'account', 'delete'),
    ('accounts:delegate', 'Delegate Accounts', 'account', 'delegate'),

    -- Budget management
    ('budgets:read', 'Read Budgets', 'budget', 'read'),
    ('budgets:write', 'Write Budgets', 'budget', 'write'),
    ...
    -- =============================================================================
    ('roles:read', 'View Roles', 'role', 'read'),
    ('roles:write', 'Create/Modify Roles', 'role', 'write'),
    ('roles:delete', 'Delete Roles', 'role', 'delete'),
    ('permissions:read', 'View Permissions', 'permission', 'read'),
    ('permissions:write', 'Create/Modify Permissions', 'permission', 'write'),
    ...
    ('user-roles:assign-basic', 'Assign Basic Roles', 'user-role', 'assign-basic'),
    ('user-roles:assign-elevated', 'Assign Elevated Roles', 'user-role', 'assign-elevated'),
    ('user-roles:revoke', 'Revoke User Roles', 'user-role', 'revoke');
...
INSERT INTO role_permissions (role_id, permission_id) VALUES
    ...
    ('ORG_ADMIN', 'users:delete'),
    -- Business data (read-only for oversight)
    ('ORG_ADMIN', 'transactions:read'),
    ('ORG_ADMIN', 'accounts:read'),
    ('ORG_ADMIN', 'budgets:read'),
    ('ORG_ADMIN', 'audit:read'),
    ('ORG_ADMIN', 'reports:export'),
    -- Role management (limited)
    ('ORG_ADMIN', 'roles:read'),
    ('ORG_ADMIN', 'user-roles:assign-basic'),
    ('ORG_ADMIN', 'user-roles:revoke');
