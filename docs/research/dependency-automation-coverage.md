...
| Orchestration | `.github/workflows/dependency-automation-config.yml`, manual dispatch with `run_hosted_dry_run=true` or the trial-only marked-push fallback | Local platform mode cannot perform a true full dry run or GitHub lookups. GitHub does not expose **Run workflow** while the workflow file exists only on the non-default trial branch. | When the UI control is available, dispatch on the exact trial ref. Otherwise publish a reviewed qualifying trial-branch push whose head commit contains `[run-hosted-renovate-dry-run]`; this runs the same read-only job and suppresses the exact-image job for that push. Retain the run URL and logs proving strict validation, `--dry-run=full`, read-only permissions, effective preset resolution, GitHub lookups, branch-file mutation simulation, registry retries, and no fatal/error output. Stop using the marker to disable it; remove both temporary marker clauses during promotion. | **Pending — [run 34831702528](https://github.com/budgetanalyzer/orchestration/actions/runs/34831702528) validated configuration, but its `dry run` head commit omitted the required marker and the hosted dry-run job was skipped** |
