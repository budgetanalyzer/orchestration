...
before publishing or triggering hosted work. The operator selected a bounded
trial with `main` remaining default and reported that neither GitHub nor Mend has
a payment method. GitHub documents that over-quota Actions use is blocked without
...
zero-spend control, selected branch-only mode, and current public artifact
snapshot. No hosted execution evidence has been supplied. In particular, this
report has no read-only hosted dry-run URL, published-preset resolution log, App
...
Those rows remain `pending`; none is being reclassified as a scanner limitation
or a pass.
...
| Orchestration | `.github/workflows/dependency-automation-config.yml`, manual dispatch with `run_hosted_dry_run=true` or the trial-only marked-push fallback | Local platform mode cannot perform a true full dry run or GitHub lookups. GitHub does not expose **Run workflow** while the workflow file exists only on the non-default trial branch. `baseBranchPatterns` and `useBaseBranchConfig=merge` do not bootstrap a repository config absent from `main`; the branch-only workflow must supply its reviewed trial config through the generated one-repository self-hosted wrapper. | When the UI control is available, dispatch on the exact trial ref. Otherwise publish a reviewed qualifying trial-branch push whose head commit contains `[run-hosted-renovate-dry-run]`; this runs the same read-only job and suppresses the exact-image job for that push. Retain the run URL and logs proving strict validation, `--dry-run=full`, read-only permissions, effective preset resolution, GitHub lookups, branch-file mutation simulation, registry retries, and no fatal/error output. The workflow must fail unless extraction, the exact trial base, mutation simulation, `Repository result: done`, and no Renovate `ERROR`/`FATAL` records are proved. Stop using the marker to disable it; remove both temporary marker clauses during promotion. | **Pending — [run 34831702528](https://github.com/budgetanalyzer/orchestration/actions/runs/34831702528) omitted the trigger marker and skipped; [run 34832298039](https://github.com/budgetanalyzer/orchestration/actions/runs/34832298039) was misleadingly green but cloned `main`, found no config, returned `disabled-no-config`, and performed zero extraction or lookups** |
