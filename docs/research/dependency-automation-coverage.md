...
| --- | --- | --- | --- | --- |
| All 10 scoped repositories | Visibility and GitHub Actions billing/storage | The unauthenticated GitHub repository API confirmed at `2026-09-09T08:51:48Z` that all ten repositories are public and use `main`; authenticated account settings remain unavailable to agents. | Reconfirm visibility when installing the App. While all ten remain public, the private-repository Actions allowance and paid-overage check is not applicable; do not add payment information or enable billing for this rollout. | **Verified for current public visibility — recheck if visibility changes** |
| All 10 scoped repositories | Free Mend Community support and App scope | Mend's public documentation confirms that Community is free for unlimited public and private repositories, with one concurrent organization job, four-hour active scheduling, a 30-minute timeout, and hosted credential settings. Account-specific App scope and package access remain operator-owned. | During Step 6, sign in with GitHub OAuth if required and verify the GitHub App can be restricted to the scoped repositories without a paid plan, trial, or payment method. Prove authenticated Maven lookup during the pilot rather than before account access exists. | **Public free-tier preflight passed — App scope and lookup pending** |
