...
- `kubernetes/kyverno/*` — the security guardrail ClusterPolicies and the static gate (`scripts/guardrails/verify-static-security-manifests.sh`). The existing exception list already covers `local-path-storage`, which is the k3s default storage class, so the baseline runs unchanged.
