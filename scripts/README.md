...
- `scripts/dev/check-phase-7-image-pinning.sh` - Static Phase 7 Session 2 scan for orchestration-owned image refs. It fails if an orchestration-owned third-party `image:`/`FROM`/verifier image constant is missing `@sha256:` or if any unexpected `:latest` appears outside the seven allowed local Tilt images.
