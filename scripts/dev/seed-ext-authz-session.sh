...
kubectl exec -n infrastructure "$REDIS_POD" -- redis-cli -a "$REDIS_PASSWORD" --no-auth-warning HSET \
...
kubectl exec -n infrastructure "$REDIS_POD" -- redis-cli -a "$REDIS_PASSWORD" --no-auth-warning EXPIRE \
