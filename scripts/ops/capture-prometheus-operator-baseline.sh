#!/usr/bin/env bash
# Capture the upstream Prometheus Operator RBAC and permission baseline.
...
OUTPUT_FILE="${REPO_DIR}/docs/research/prometheus-operator-rbac-baseline.md"
TMP_DIR=""

usage() {
    cat <<'EOF'
Usage: ./scripts/ops/capture-prometheus-operator-baseline.sh [options]

Renders the current kube-prometheus-stack manifests without the repo-owned
post-renderer, extracts the Prometheus Operator RBAC objects, builds a
representative kubectl auth can-i matrix for the operator service account, and
writes a checked-in Markdown baseline document.

Options:
  --output PATH   Output Markdown path. Relative paths resolve from the repo
                  root. Default:
                  docs/research/prometheus-operator-rbac-baseline.md
...
log_step "Writing baseline document"
python3 - <<'PY' \
    "${RBAC_SUMMARY_FILE}" \
    "${RBAC_FILE}" \
    "${CAN_I_FILE}" \
...
lines = []
lines.append("# Prometheus Operator RBAC Baseline")
lines.append("")
lines.append(f"**Date:** {capture_date}")
lines.append("**Generated by:** `./scripts/ops/capture-prometheus-operator-baseline.sh`")
lines.append("")
lines.append(
    "This note captures the upstream `kube-prometheus-stack` operator RBAC and "
    "representative live authorization evidence before the repo-owned RBAC "
    "reduction layer is applied."
)
lines.append("")
...
lines.append("")
lines.append(
    "Run `./scripts/smoketest/verify-monitoring-runtime.sh` separately after "
    "Tilt has brought up the full app stack when you also want current "
    "runtime scrape evidence; this capture stays focused on RBAC and "
    "authorization state."
)
lines.append("")
